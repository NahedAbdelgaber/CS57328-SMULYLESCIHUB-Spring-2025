...
public class UserService {
    ...
    public boolean registerUser(User user) {
        String sql = "INSERT INTO users (email, username, password, name, address, degree, active, email_opt_in) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        jdbc.update(sql, user.getEmail(), user.getUsername(), user.getPassword(), user.getName(), user.getAddress(), user.getDegree(), true, true);
        return true;
    }
    ...
    public boolean deleteUser(String identifier) {
        String sql = "UPDATE users SET active = FALSE WHERE username = ? OR email = ?";
        int rows = jdbc.update(sql, identifier, identifier);
        return rows > 0;
    }

    public boolean deactivateUser(String identifier) {
        return deleteUser(identifier);
    }

    public boolean updateEmailOptIn(String identifier, boolean optIn) {
        String sql = "UPDATE users SET email_opt_in = ? WHERE (username = ? OR email = ?)";
        int rows = jdbc.update(sql, optIn, identifier, identifier);
        return rows > 0;
    }
}
