...
public class UserController {
    ...
    public ResponseEntity<?> getProfile(@PathVariable String identifier) {
        ...
        dto.setEmailOptIn(user.getEmailOptIn());
    ...
    public ResponseEntity<?> deactivate(@RequestBody LoginRequest req) {
        String id = (req.getUsername()!=null&&!req.getUsername().isEmpty())?req.getUsername():req.getEmail();
        boolean ok = userService.deactivateUser(id);
        return ok ? ResponseEntity.ok().build() : ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to deactivate");
    }

    @PutMapping("/profile/{identifier}/email-opt-in")
    public ResponseEntity<?> setEmailOptIn(@PathVariable String identifier, @RequestBody Map<String,Boolean> body) {
        Boolean optIn = body.get("emailOptIn");
        if (optIn == null) {
            return ResponseEntity.badRequest().body("Missing 'emailOptIn' flag");
        }
        boolean ok = userService.updateEmailOptIn(identifier, optIn);
        return ok ? ResponseEntity.ok().build() : ResponseEntity.status(500).body("Failed to update preference");
    }
}
